...
Orders a server with [`eq/order_instance`](https://hostkey.ru/documentation/apidocs/eq/#order_instance), waits for deploy, manages hostname, tags, power, reboot and reinstall. Destroy calls `whmcs/request_cancellation`.
...
`terraform destroy` calls [`whmcs/request_cancellation`](https://hostkey.ru/documentation/apidocs/whmcs/#request_cancellation). Like the Hostkey panel, each server **must** declare:
...
- `location_name` (String) Data-center code (`NL`, `US`, `FI`, `DE`, `RU`, …). This provider always uses `invapi.hostkey.ru` (not the `.com` portal).
...
- RAID / disk layout: [документация RAID](https://hostkey.ru/documentation/technical/exist_server_using/raid_create/).
