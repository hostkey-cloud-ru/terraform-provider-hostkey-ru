...
func pendingDeployWarningTitleDetail(err error, invoice int, callback, pendingID string) (title, detail string) {
	...
	if invapi.IsPendingPayment(err) {
		...
		detail = fmt.Sprintf(
			"%v. Apply waited for payment until create timeout; Pay this invoice in the Hostkey panel (Profile → Billing / Invoices; enable auto-pay from credit balance or top up). State kept as %s. Re-run terraform apply after payment — it resumes this invoice and will not place a new order. Or keep the first apply running and pay while it still shows Still creating...",
			err, id,
		)
		return title, detail
	}
